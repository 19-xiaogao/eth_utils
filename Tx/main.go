...
func NewTx(client *ethclient.Client) *Tx {
	return &Tx{client: client}
}

// SendEthTx 使用私钥发送交易
func (server *Tx) SendEthTx(privateKey string, recipientAddress string, amount interface{}) error {

	_privateKey, _ := crypto.HexToECDSA(privateKey)

	fromAddress := wallet.PrivateToAddress(privateKey)
...
func (server *Tx) Distribute(privateKey string, address []string, amount float64) error {
	...
	for i := 0; i < len(address)-1; i++ {
		if !wallet.IsValidAddress(address[i]) {
			fmt.Println("")
			continue
		}
		//go func() {
...
func (server *Tx) Collection(privateList []string, collectionAddress string) error {
	gasLimit := uint64(21000)
	...
	for i := 0; i < len(privateList)-1; i++ {
		...
		fromAddress := wallet.PrivateToAddress(privateList[i])
		balance, err := server.client.BalanceAt(context.Background(), common.HexToAddress(fromAddress), nil)
		if err != nil {
			return err
		}
